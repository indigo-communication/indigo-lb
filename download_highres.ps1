...
Write-Host "Found $($urlMap.Count) unique images to download" -ForegroundColor Cyan
...
foreach ($entry in $urlMap.GetEnumerator()) {
    ...
    if (Test-Path $outputPath) {
        ...
        if ($currentSize -gt 100KB) {
            ...
            Write-Host "  Skipped (already HR): $filename - $sizeKB KB" -ForegroundColor Gray
            $skipped++
            continue
        }
    }
...
Write-Host "`n=== DOWNLOAD COMPLETE ===" -Fore groundColor Green
...
Write-Host "`n=== NEW STATS ===" -ForegroundColor Cyan
Write-Host "  High-res: $($highRes.Count)" -ForegroundColor Green
Write-Host "  Medium-res: $($mediumRes.Count)" -ForegroundColor Yellow
Write-Host "  Low-res: $($lowRes.Count)" -ForegroundColor $(if ($lowRes.Count -gt 20) { 'Red' } else { 'Green' })
